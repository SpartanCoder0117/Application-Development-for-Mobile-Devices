...
public class Lienzo extends View{
    ...
    protected void onDraw(Canvas c){
        ...
        for(int i=1; i<x; i++) r.lineTo(i, (float) Math.sin(i / 50f) * (-100f)); //i/20f * (-80f) (periodo * amplitud)
        r.offset(-10, y0); c.drawPath(r, p);

        r = new Path();
        r.moveTo(0, 0); p.setColor(Color.RED);
        for(int i=1; i<x; i++) r.lineTo(i, (float) Math.cos(i / 40f) * (-200f)); //i/20f * (-100f)
        r.offset(-10, y0);
        c.drawPath(r, p);

        /*p.setColor(Color.rgb(255, 255, 0)); // Ejes azules
        c.drawLine(x0, 0, x0, y, p);
        c.drawLine(0, y0, x, y0, p);*/
    }
}