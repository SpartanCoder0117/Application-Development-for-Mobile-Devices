 ...
 * @author inkis
 */
public class MiPiramide extends MIDlet implements CommandListener {
    ...
    public MiPiramide() {
        ...
        ca = new Canvas(){
            ...
            void line(Graphics g, int i, int j){
                Point2D2 p = obj.vScr[i], q = obj.vScr[j];
...
class Obj2{	// Posee los datos del objeto 3D
    float rho, theta=0.3F, phi=1.3F, d, objSize, v11, v12, v13, v21, v22, v23, v32, v33, v43; // elementos de la matriz V
    Point3D2 [] w;	// coordenadas universales
    Point2D2 [] vScr; // coordenadas de la pantalla
    Obj2(){	// CAMBIAR LAS COORDENADAS X,Y,Z CON 0,1 PARA CONSTRUIR PRISMA, CILINDRO, PIRAMIDE, CONO Y ESFERA.
        w	= new Point3D2[8];
	vScr	= new Point2D2[8];
        
        //Piramide
        w[0] = new Point3D2(1, -1, -1); // desde la base
	w[1] = new Point3D2(1, 1, -1);
	w[2] = new Point3D2(-1, 1, -1);
	w[3] = new Point3D2(-1, -1, -1);
	w[4] = new Point3D2(0, 0, 1);
	w[5] = new Point3D2(0, 0, 1);
	w[6] = new Point3D2(0, 0, 1);
	w[7] = new Point3D2(0, 0, 1);
	...
	for(int i=0; i<8; i++){
            ...
            vScr[i] = new Point2D2(-d*x/z, -d*y/z);
	}
    }
}
...
class Point3D2{
    float x, y, z;
    Point3D2(double x, double y, double z){
        this.x = (float) x;
	this.y = (float) y;
	this.z = (float) z;
    }
